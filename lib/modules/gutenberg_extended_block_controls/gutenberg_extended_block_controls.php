...
class gutenberg_extended_block_controls extends modules {
	...
	protected function register_scripts(): gutenberg_extended_block_controls {
		if ($this->is_active() === true) {
			...
			$this->get_script('stretch_link')->set_path('lib/frontend/css/common/stretch_link.css')->set_inline();
		}
	...
	public function render_block_overwrite(string $block_content, array $block): string {
		...
		// mod css
		if (isset($block['attrs']['_classNamesList']) && in_array(
			                                                 'sv100-premium-block-core-mod-flex',
			                                                 $block['attrs']['_classNamesList']
		                                                 ) === true) {
	...
	public function get_frontend_block_styles(bool $as_return = false) {
		...
		foreach ($blocks as $block) {
			if (isset($block['attrs']) === false || isset($block['attrs']['parsedCSSString']) === false) {
				continue;
			}
